...
DELIVERY_DIR = os.path.join(ROOT_DIR, 'DELIVERY', 'TARGET_CY8CKIT_062_WIFI_BT_PSA')
...
for f in fnmatch.filter(os.listdir(DELIVERY_DIR), '*.hex'):
    ...
    if os.path.exists(test_directory):
        target_dir = os.path.join(test_directory, 'TARGET_CY8CKIT_062_WIFI_BT_PSA', f)
        if not os.path.exists(os.path.join(test_directory, 'TARGET_CY8CKIT_062_WIFI_BT_PSA')):
            continue
    else:
