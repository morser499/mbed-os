...
static int expand_dct( FILE *dct_file, long newsize )
{
    ...
    if ( seek_result != 0 )
    {
        WPRINT_PLATFORM_INFO( ("Couldn't seek to end of DCT file\n") );
        return -2;
    }
    ...
    if ( zeros == NULL )
    {
        WPRINT_PLATFORM_INFO( ("Couldn't malloc space to expand DCT\n") );
        return -3;
    }
    ...
    {
        WPRINT_PLATFORM_INFO( ("Error expanding DCT\n") );
        free( zeros );
        return -4;
    }
    free( zeros );
    ...
    {
        WPRINT_PLATFORM_INFO( ("Error expanding DCT memory\n") );
        free( dct_memory );
        return -5;
    }
...
static wiced_result_t dct_init( void )
{
    ...
    if ( dct_file == NULL )
    {
        WPRINT_PLATFORM_INFO( ("Creating DCT file \"%s\"\n",DCT_FILENAME) );

        dct_file = fopen( DCT_FILENAME, "w+b" );
        if ( dct_file == NULL )
        {
            WPRINT_PLATFORM_INFO(( "Error opening DCT file \"%s\"\n",DCT_FILENAME) );
            return WICED_ERROR;
        }
    }
    ...
    {
        WPRINT_PLATFORM_INFO( ("Error DCT file is too big\n") );
        fclose( dct_file );
        return WICED_ERROR;
    }
    ...
    if ( dct_memory == NULL )
    {
        WPRINT_PLATFORM_INFO( ("Could not malloc space for DCT memory image\n") );
        fclose( dct_file );
        return WICED_ERROR;
    }
    ...
    if ( seek_result != 0 )
    {
        WPRINT_PLATFORM_INFO( ("Couldn't seek to start of DCT file\n") );
        fclose( dct_file );
        return -10;
    }
    ...
    {
        WPRINT_PLATFORM_INFO( ("Error - could not read all of DCT\n") );
        fclose( dct_file );
        return WICED_ERROR;
    }
         ...
         ( dct_hdr->magic_number != BOOTLOADER_MAGIC_NUMBER ) )
    {
        ...
        if ( write_result != 0 )
        {
            WPRINT_PLATFORM_INFO( ("Error initialising blank DCT") );
            return WICED_ERROR;
        }
    }
...
static int platform_write_dct( uint16_t data_start_offset, const void* data, uint16_t data_length, int8_t app_valid, void (*func)(void) )
{
    ...
    {
        WPRINT_PLATFORM_INFO( ("Attempt to write beyond the maximum DCT length\n") );
        WICED_DCT_MUTEX_RELEASE();
        return -1;
    }
    ...
    if ( seek_result != 0 )
    {
        WPRINT_PLATFORM_INFO( ("Error seeking to start of DCT write") );
        fclose( dct_file );
        WICED_DCT_MUTEX_RELEASE();
        return -2;
    }
    ...
    {
        WPRINT_PLATFORM_INFO( ("Error writing DCT\n") );
        fclose( dct_file );
        WICED_DCT_MUTEX_RELEASE();
        return -5;
    }
    ...
    if ( seek_result != 0 )
    {
        WPRINT_PLATFORM_INFO( ("Error seeking to start of DCT") );
        fclose( dct_file );
        WICED_DCT_MUTEX_RELEASE();
        return -6;
    }
    ...
    {
        WPRINT_PLATFORM_INFO( ("Error re-reading DCT after write\n") );
        WICED_DCT_MUTEX_RELEASE();
        return -7;
    }
