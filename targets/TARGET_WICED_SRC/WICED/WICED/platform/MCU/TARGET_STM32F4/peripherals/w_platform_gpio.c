...
typedef struct
{
    ...
    platform_gpio_IRQHandler_callback_t handler;    // User callback
    void*                        arg;        // User argument to be passed to the callbackA
} platform_gpio_IRQHandler_data_t;
...
static volatile platform_gpio_IRQHandler_data_t gpio_IRQHandler_data[NUMBER_OF_GPIO_IRQ_LINES];
...
platform_result_t platform_gpio_IRQHandler_enable( const platform_gpio_t* gpio, platform_gpio_IRQHandler_trigger_t trigger, platform_gpio_IRQHandler_callback_t handler, void* arg )
{
    ...
    if ( ( EXTI->IMR & interrupt_line ) == 0 )
    {
        ...
        gpio_IRQHandler_data[gpio->pin_number].owner_port = gpio->port;
        gpio_IRQHandler_data[gpio->pin_number].handler    = handler;
        gpio_IRQHandler_data[gpio->pin_number].arg        = arg;
...
platform_result_t platform_gpio_IRQHandler_disable( const platform_gpio_t* gpio )
{
    ...
    if ( ( EXTI->IMR & interrupt_line ) && gpio_IRQHandler_data[gpio->pin_number].owner_port == gpio->port )
    {
        ...
        gpio_IRQHandler_data[gpio->pin_number].owner_port = 0;
        gpio_IRQHandler_data[gpio->pin_number].handler    = 0;
        gpio_IRQHandler_data[gpio->pin_number].arg        = 0;
...
platform_result_t platform_gpio_deepsleep_wakeup_enable( const platform_gpio_t* gpio, platform_gpio_IRQHandler_trigger_t trigger )
{
...
platform_result_t platform_gpio_IRQHandler_manager_init( void )
{
    memset( (void*)gpio_IRQHandler_data, 0, sizeof( gpio_IRQHandler_data ) );
...
WWD_RTOS_DEFINE_ISR( gpio_IRQHandler )
{
    ...
    if ( gpio_IRQHandler_data[gpio_number].handler != NULL )
    {
        void * arg = gpio_IRQHandler_data[gpio_number].arg; /* Avoids undefined order of access to volatiles */
        gpio_IRQHandler_data[gpio_number].handler( arg );
    }
}
...
WWD_RTOS_MAP_ISR( gpio_IRQHandler , EXTI0_IRQHandler     )
WWD_RTOS_MAP_ISR( gpio_IRQHandler , EXTI1_IRQHandler     )
WWD_RTOS_MAP_ISR( gpio_IRQHandler , EXTI2_IRQHandler     )
WWD_RTOS_MAP_ISR( gpio_IRQHandler , EXTI3_IRQHandler     )
WWD_RTOS_MAP_ISR( gpio_IRQHandler , EXTI4_IRQHandler     )
WWD_RTOS_MAP_ISR( gpio_IRQHandler , EXTI9_5_IRQHandler   )
WWD_RTOS_MAP_ISR( gpio_IRQHandler , EXTI15_10_IRQHandler )
