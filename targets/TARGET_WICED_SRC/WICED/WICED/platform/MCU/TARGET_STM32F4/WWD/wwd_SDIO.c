...
static void sdio_oob_IRQHandler_handler( void* arg )
{
...
static void sdio_enable_bus_IRQHandler( void )
{
#if defined(STM32F412xG)
...
static void sdio_disable_bus_IRQHandler( void )
{
    SDIO->MASK = 0;
}
...
wwd_result_t host_enable_oob_interrupt( void )
{
    ...
    platform_gpio_IRQHandler_enable( &wifi_sdio_pins[WWD_PIN_SDIO_OOB_IRQ], IRQ_TRIGGER_RISING_EDGE, sdio_oob_IRQHandler_handler, 0 );
    return WWD_SUCCESS;
}
...
wwd_result_t host_platform_bus_init( void )
{
    ...
    w_SDIO_Init( &sdio_init_structure );
    w_SDIO_SetPowerState( SDIO_PowerState_ON );
    w_SDIO_SetSDIOReadWaitMode( SDIO_ReadWaitMode_CLK );
    w_SDIO_ClockCmd( ENABLE );
...
wwd_result_t host_platform_bus_deinit( void )
{
    ...
    sdio_disable_bus_IRQHandler( );
    w_SDIO_ClockCmd( DISABLE );
    w_SDIO_SetPowerState( SDIO_PowerState_OFF );
    w_SDIO_DeInit( );
...
WWD_RTOS_DEFINE_ISR( sdio_dma_IRQHandler )
{
    wwd_result_t result;
...
WWD_RTOS_MAP_ISR( sdio_IRQHandler,     SDIO_IRQHandler         )
WWD_RTOS_MAP_ISR( sdio_dma_IRQHandler, DMA2_Stream3_IRQHandler )
