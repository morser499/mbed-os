...
typedef struct
{
    platform_gpio_IRQHandler_callback_t handler;  /* User callback function for this GPIO IRQ */
    void*                        arg;      /* User argument passed to callback function */
    /* What triggers interrupt, e.g. high level, low level, raising edge, etc */
    platform_gpio_IRQHandler_trigger_t  trigger;
...
platform_result_t platform_gpio_irq_enable( const platform_gpio_t* gpio, platform_gpio_IRQHandler_trigger_t trigger, platform_gpio_IRQHandler_callback_t handler, void* arg )
{
    GPIO_PRT_Type *base = NULL;
...
#define IRQ_HANDLER_MAP(a) \
     WWD_RTOS_MAP_ISR( platform_gpio_port_##a##_irq , ioss_interrupts_gpio_##a##_IRQHandler)
